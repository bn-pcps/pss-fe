<script>
	...
	let isEnabled = false;
	let password = '';
	// $share.extra_features.password = '';
  // $share.extra_features.enabled_features = [...]
	// let password = $derived($share.extra_features.password || '');

	function updateSharePassword() {
		share.update((current) => {
			const features = current.extra_features.enabled_features || [];
			let newFeatures = features;
			if (isEnabled) {
				// Add 'password' if not present
				if (!features.includes('password')) {
					newFeatures = [...features, 'password'];
				}
			} else {
				// Remove 'password' if present
				newFeatures = features.filter((f) => f !== 'password');
			}
			return {
				...current,
				extra_features: {
					...current.extra_features,
					password: isEnabled ? password : undefined,
